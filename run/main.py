...
from config import spo_config_v2,spo_config_v1

from run.data_loader import Reader, Feature
...
def build_dataset(args, spo_config, reader, tokenizer, debug=False):
...
def main():
    ...
    if not os.path.exists(args.output):
        logger.info('\'{}\' does not exist. mkdir it.'.format(args.output))
        os.makedirs(args.output)
    if not os.path.exists(args.cache_data):
        logger.info('\'{}\' does not exist. mkdir it.'.format(args.cache_data))
    ...
    eval_examples, data_loaders, tokenizer = build_dataset(args, spo_conf, reader,tokenizer, debug=False)
